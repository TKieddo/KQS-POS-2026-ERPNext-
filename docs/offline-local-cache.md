...
> Shipped for **one till per warehouse** and outages of **minutes to a few hours**.
...
2. Only **one till** may hold the offline lease for a warehouse.
...
Warehouse Offline Lease  →  one till per store warehouse
...
1. Open POS online → cache pull (banner clears).
...
- Not multi-till offline; not multi-day offline
